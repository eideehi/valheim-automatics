...
namespace Automatics.AutomaticDoor
{
    ...
    internal static class Config
    {
        ...
        public static void Initialize()
        {
            ...
            _allowAutomaticDoorCustom =
                config.Bind("allow_automatic_door_custom", new StringList());
            _intervalToOpen = config.Bind("interval_to_open", 0.1f, (0f, 8f));
            _intervalToClose = config.Bind("interval_to_close", 0.1f, (0f, 8f));
            _distanceForAutomaticOpening =
                config.Bind("distance_for_automatic_opening", 2.5f, (1f, 8f));
            _distanceForAutomaticClosing =
                config.Bind("distance_for_automatic_closing", 2.5f, (1f, 8f));
            _automaticDoorEnableDisableToggle = config.Bind("automatic_door_enable_disable_toggle",
                new KeyboardShortcut());
            _automaticDoorEnableDisableToggleMessage =
                config.Bind("automatic_door_enable_disable_toggle_message", Message.Center);
