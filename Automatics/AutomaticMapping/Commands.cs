...
namespace Automatics.AutomaticMapping
{
    internal static class Commands
    {
        ...
        private class PrintObjects : Command
        {
            private int _radius;
            private int _number;
            ...
            private void PrintObject(Terminal.ConsoleEventArgs args, string type,
                ...
                Predicate<MonoBehaviour> predicate)
            {
                var objectType =
                         ...
                         select (x, distance.ToString("F1")))
                {
                    ...
                    var message = defined
                        ? Automatics.L10N.LocalizeTextOnly(
                            ...
                            localizedName, name, objectType, obj.transform.position, distance, identifier)
            ...
            {
                PrintObject(args, "mineral", ValheimObject.Mineral, GetObjects(),
                    x => x.GetComponent<MineRock>() || x.GetComponent<MineRock5>() ||
                         x.GetComponent<Destructible>());
            }

            private void PrintSpawner(Terminal.ConsoleEventArgs args)
            {
                PrintObject(args, "spawner", ValheimObject.Spawner, GetObjects(),
                    x => x.GetComponent<SpawnArea>());
            }
            ...
            {
                PrintObject(args, "other", MappingObject.Other, GetObjects(),
                    x => x.GetComponent<IDestructible>() is MonoBehaviour ||
                         x.GetComponent<Interactable>() is MonoBehaviour ||
                         x.GetComponent<Hoverable>() is MonoBehaviour);
            }
            ...
            private IEnumerable<MonoBehaviour> GetObjects()
            {
                ...
                for (var i = 0; i < size; i++)
                {
                    ...
                    var obj = collider.GetComponentInParent<MonoBehaviour>();
                    if (obj)
                        result.Add(obj);
                }
